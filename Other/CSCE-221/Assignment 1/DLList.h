...
public:
  ...
  DLList(DLList& dll); // copy constructor
  DLList(DLList&& dll); // move constructor
  ~DLList(); // destructor
  DLList& operator=(DLList& dll); // copy assignment operator
  ...
  DLListNode getHeader() const {return header;}
  DLListNode getTrailer() const {return trailer;}
};

// output operator
ostream& operator<<(ostream& out, DLList& dll);
